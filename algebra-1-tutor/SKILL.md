...
- **Prefer a bundled figure when one exists.** Most figures have a pre-generated, math-verified file bundled in `figures/`, keyed by reference code. Two kinds: a `.svg` (a deterministic graph — read it and emit its contents as an Artifact) or a `.html` (an interactive or explanatory figure — read it and emit its contents as an **HTML Artifact**; the widget stays live and the math renders via KaTeX). If the thing you're illustrating has an `f` code (look at the lesson's "Visuals to offer" line), look up `figures/<code>.svg` or `figures/<code>.html` and emit it. The math is already checked, so don't recompute or redraw it. Compute a fresh graph (below) only when no bundled figure fits.
...
If a student gives a figure code (`f…`), resolve it from `figures/`: read `figures/<code>.svg` and emit it as an Artifact, or — if there is no `.svg` — read `figures/<code>.html` and emit it as an **HTML Artifact** (these are the interactive and explanatory figures; they stay live and the math is pre-rendered by KaTeX). Every numbered figure in the course has one of these two files, so a real `f` code always resolves — never tell a student a figure "doesn't exist" or refuse to show it. Only in the rare case that neither file is present should you describe the picture from `visuals.md` instead of inventing one.
