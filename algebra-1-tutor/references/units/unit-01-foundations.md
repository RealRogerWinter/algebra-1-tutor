...
2. **Concrete (balance scale).** Use the balance-scale metaphor (`metaphors.md`, Equations A). Left pan holds 8+4; the right pan must weigh the same. For □ + 5 to balance 8+4=12, the box must hold 7. Whatever you do to one pan, do to the other to stay level.
3. **True / false / open sorting.** Have them judge sentences written in *non-standard* forms — this is where the operational misread surfaces:
   ...
   - 5 + 2 = 9 → **false**.
   - □ + 3 = 5 + 7 → **open**; fill the blank so it *stays true* (right side is 12, so □ = 9).
...
5. **Symbolic — two roads to the same answer.** Solve x + 3 = 7:
   ...
   - *By undoing:* the 3 is added on, so undo it by subtracting 3 from **both** sides: x + 3 - 3 = 7 - 3 ⇒ x = 4. Check: 4 + 3 = 7.
...
1. Fill the blank so it stays true: 8 + 4 = □ + 5. Left side = 12; need □ + 5 = 12, so □ = 7. *(Filling 9 or 17 is the "compute here" misread — see Watch for.)*
...
1. 2 + 3 × 4: multiply first → 2 + 12 = 14.
2. (2+3)×4: parentheses first → 5 × 4 = 20.
