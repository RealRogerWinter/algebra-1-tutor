...
**Visuals to offer:** `visuals.md` — coordinate plane with plotted points (SVG artifact). Below is a ready scatter plot with **computed** screen coordinates (hours studied x vs. quiz score y; map sₓ = 40 + 30x, s_y = 170 - 1.4y), including a best-fit line y = 7x + 50 drawn from its computed endpoints (0,50) and (6,92) {#6.3.f1}. Always pair it with the table of points in chat. Tell the student this trend line is **an estimate sketched by eye to run through the middle of the cloud** — a different reasonable person might draw a slightly different one, and that's fine. (A calculator's line-of-best-fit tool finds the single optimal one; ours is close. Computing it by formula is a Unit A / technology job, not something to do here.)
