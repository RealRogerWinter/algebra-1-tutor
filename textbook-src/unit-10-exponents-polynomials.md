...
> **What's worth having fresh:** from Unit 1, exponents as repeated multiplication, order of operations, and the difference between −3² and (−3)². From Unit 2, the distributive property, combining like terms, and the habit of handing the minus sign to *every* term inside parentheses. None of it is heavy; a quick redo of two or three of those problems from memory before you start will make this unit feel familiar.
> **By the end of this unit, you'll be able to:**
> - Simplify expressions with the product, quotient, power, and power-of-a-product rules, and say *why* x⁰ = 1 and x⁻ᵃ = 1/(xᵃ) have to be true.
> - Write very large and very small numbers in scientific notation, and multiply and divide them in that form.
...
> - Multiply a monomial by a polynomial, and a binomial by a binomial using an area box, getting the middle term right every time.

This unit opens the back third of the course, and it all points one direction: building expressions up. You'll learn to multiply things out — to take (x + 2)(x + 3) and turn it into x² + 5x + 6. The very next unit runs the same machinery backward, so the work you put in here pays off twice. Before each new lesson, redo two or three problems from a lesson or two earlier from memory; that small warm-up keeps the older moves sharp while the new ones land.

---

## Lesson 10.1: Exponent rules (including zero & negative exponents)

Exponents are just a shorthand for "multiply this by itself a few times." That's the whole idea, and every rule in this lesson comes straight out of it. The trick is to never let the shorthand pull you away from what it's counting. When you're unsure what a rule should be, you can always write the factors out and count them — and that habit, more than any memorized rule, is what keeps you out of trouble here.
...
Line up the x's: two of them, then three more, five in all. When you multiply two powers of the same base, you're pooling all their factors into one pile, so the exponents **add**. That single picture — pool the factors, count them — is the thing to hold onto, because it settles the most common mix-up in the whole lesson before it can start.
...
Three copies of x², so 2 + 2 + 2 = 6. Here the exponents **multiply**. Sit with the contrast for a second, because it's the heart of the lesson: x² · x³ is two *separate* powers multiplied, so you add (2 + 3 = 5); (x²)³ is *one* power taken three times, so you multiply (2 · 3 = 6). Same-looking, opposite arithmetic. When you can feel which situation you're in, you've got the lesson.
...
Negative exponents come from the same move, just with more on the bottom than the top. Take x² over x⁵, and again read it two ways. The quotient rule gives x²⁻⁵ = x⁻³. Counting factors gives something you can see: two factors on top pair off with two on the bottom and go to one, leaving three factors stranded *on the bottom* — that's 1/(x³). Same quantity, two descriptions:
...
The slip that catches the most people here is using the wrong rule between two that look alike: writing x² · x³ = x⁶, or (x²)³ = x⁵. The pull is to run the same operation in both cases, since the expressions look so similar. The fix is always the same, and you already have it: write the x's out and count. x² · x³ is (xx)(xxx), five x's, so x⁵. (x²)³ is x² three times over, six x's, so x⁶. The quick check is to ask yourself, "am I multiplying two separate powers, or raising one power to another?" — add for the first, multiply for the second.

One more place to watch is the power-of-a-product rule, because it's easy to leave the coefficient behind. In (2x)³, the exponent greets *everything* inside the parentheses, the 2 included: it's 2³x³ = 8x³, not 2x³. The 3 is sitting on the whole 2x, not just the x. The same question settles its zero-exponent cousin: in 5x⁰ there are no parentheses, so the exponent touches only the x, giving 5 · 1 = 5; but (5x)⁰ puts the whole 5x under the exponent, giving 1. Before you apply a 0 or a power, ask what the base actually is — just the x, or the whole thing?

Try one clean problem before the practice mixes things up. Simplify y⁵ · y. The lone y is y¹, so add the exponents: y⁵ · y¹ = y⁶. Nothing subtle there: just the product rule, once, with the silent exponent of 1 made visible.
...
(Item 18 uses a negative exponent as an *input* rather than an answer: x⁵·x⁻² = x⁵⁺⁽⁻²⁾ = x³ — the product rule with a negative exponent, the reverse of items 14–15 where the negative exponent was the *result*. The answer is positive because 5 + (−2) = 3.)
...
Some numbers are too long to read comfortably — the distance to a star, the width of an atom. Scientific notation is a tidy way to write them: one digit, a decimal tail, and a power of ten that records how big the number really is. The good news is you already did the hard part last lesson. Multiplying and dividing these numbers is just the exponent rules from 10.1, applied to powers of ten.
...
Now that you've seen it done, two slips are worth naming. The first is leaving the coefficient out of range. You might write 53 × 10² or 0.53 × 10⁴ for 5300, and both *equal* 5300, but neither is scientific notation, because the coefficient has to be at least 1 and under 10. The second shows up after multiplying or dividing: it's tempting to stop at 30 × 10⁷ and call it done, since the value is right. The value is right; the form isn't. Ask yourself the window question every time. Is my coefficient at least 1 and under 10? If not, slide and adjust. And for small numbers, the sign of the exponent is easy to get backward, so run a quick test: is the number bigger or smaller than 1? Smaller than 1 means a negative power of ten.
...
These problems jump between converting, multiplying, and dividing, which is the kind of switching that builds a lasting skill. Each answer is at the end of the lesson, and the worked examples above cover every move they ask for, so flip back whenever one stalls you. The last three deliberately push the coefficient out of range so you get practice sliding it home.
...
A polynomial is just a sum of terms like 3x² + 2x − 1 — number-times-a-power pieces, added up. It's the noun the next three units are about, so this lesson gives the parts their names and then does the two easiest things you can do with them: add and subtract. The adding is nothing new; it's Unit 2's "combine like terms" with fresh vocabulary. The subtracting hides one genuine trap, and most of the lesson's care goes there.
...
Watch the two that bite: the −x inside became +x, and the +2 became −2. The first term flipping is obvious; it's these later ones that get left alone by accident. And here's the move that catches it for you — put a simple number in for x and check both sides. At x = 1, the original is (2 + 3) − (1 − 1 + 2) = 5 − 2 = 3, and the result is 1 + 4 − 2 = 3. They match, so the signs are right. If they *hadn't* matched, that wouldn't be a failure — your check would have just caught a slipped sign before it cost you anything. The next move would be to go back and re-run the flips one term at a time, because a mismatch here is almost always one sign left unflipped, not a broken method.
...
The slip to keep an eye on is the one the subtraction examples are built around: flipping only the first sign after a minus. It's easy to write (2x² + 3x) − (x² − x + 2) as x² + 2x + 2, leaving the −x and the +2 alone. The cause is natural — the parentheses make it feel like the minus belongs to the whole group rather than to each piece. The cure is the substitution check you just saw: a mismatch at x = 1 points straight at a sign you didn't flip. The other thing people do is fuse unlike terms, writing 3x² + 2x as 5x² or 5x³. Those are different kinds of box; an x²-piece and an x-piece can't be stacked, so 3x² + 2x just stays as it is.

Settle on one clean problem before the set mixes things up: name the type, degree, and leading coefficient of 7x⁵. One term, so it's a monomial; its highest (and only) power is 5, so degree 5; and the number on that term is 7, the leading coefficient. You're just reading the parts off the page.
...
3. {#10.3.c3} Why can't 3x² and 3x be combined, even though both have a 3 and an x? (They're different kinds of term — one is an x²-piece, the other an x-piece — and only terms with the same variable *and* the same power combine.)

You can now name a polynomial's parts, write it in standard form, add by combining like terms, and subtract with every sign flipped — and check that subtraction with a quick substitution so a missed sign never slips past you.

The problems below shuffle naming, adding, and subtracting together, the kind of switching that makes a skill stay put. Each answer is at the end of the lesson, and the worked examples above match every problem here. The subtraction problems are where the sign trap lives, so run a substitution check on each one. That habit is the main thing this lesson is teaching.
...
This is the forward direction of the course's main thread: you take (x + 2)(x + 3) and multiply it out to x² + 5x + 6. The next unit hands you x² + 5x + 6 and asks you to find the pieces it came from — so multiplying out now is the very thing you'll learn to undo. There's one spot where almost everyone loses a piece, the *middle* term, and the area box in this lesson is built to make that piece impossible to miss.
...
There they are: four cells, x², 3x, 2x, and 6. The two in the middle, 3x and 2x, are the ones people drop — and they add up to the 5x that makes this a trinomial instead of just x² + 6. You may have heard of FOIL; it's just these same four products in a fixed order. The box is worth preferring, because it *shows you why a middle term exists* and keeps working when the pieces get bigger, where the acronym runs out.
...
Two of those deserve a second look. In the difference-of-squares example, the two middle cells, +4x and −4x, go to zero together, which is the whole reason (x − 4)(x + 4) collapses to just x² − 16. And in the last one, squaring a binomial, the tempting move is to square each piece and write x² + 9. But (x + 3)² means (x + 3)(x + 3), and running the box gives a middle term of 3x + 3x = 6x. So (x + 3)² is x² + 6x + 9; the 6x is exactly the part that gets dropped, and catching it is what the example is for.

Across all of these, the error to watch for is the missing middle term: writing (x + 2)(x + 3) as x² + 6, or (x + 3)² as x² + 9. It happens when only the first-and-last products get written down. The box is the cure, because it makes the question concrete. Every part of the width meets every part of the height, so there are *four* cells, not two; if you only have two terms, ask where the middle cells went. When a sign is in play, fill each cell with its *signed* product (x · −4 is −4x) and then add, and a substitution check at x = 1 will flag any sign that went astray.

Run the box on one clean problem before the set mixes things up. Expand (x + 5)(x + 1). The four cells are x², x, 5x, and 5; the middle two, x and 5x, add to 6x; so it's x² + 6x + 5. Four cells, then collect the middle pair, and that's the whole method.
...
3. {#10.4.c3} Suppose someone writes (x + 4)² = x² + 16. Show them — with the box — exactly what they dropped. (Running (x + 4)(x + 4) gives middle cells 4x and 4x; they add to 8x, so the answer is x² + 8x + 16 — the dropped piece is the 8x.)
...
The set below mixes monomial products with binomial products instead of grouping them, which is harder in the moment and better for remembering. Each answer is at the end of the lesson, and the worked examples above cover every move below, so flip back whenever one stalls you. The last three are the special patterns: watch for a vanishing middle term in one and a doubled one in the others.
