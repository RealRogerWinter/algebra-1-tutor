...
5. Solve: x/3 = 4. This time x is divided by 3, so undo by multiplying both sides by 3, giving x = 12. Check: 12/3 = 4, which matches.
...
1. Classify 7. It's a counting number, so it's an **integer**, and 7 = 7/1 makes it **rational** too. The labels stack; a number can be several types at once.
2. Classify 4.8. It isn't a whole amount, so it's *not* an integer; but 4.8 = 24/5, a ratio of two integers, so it's **rational, non-integer**.
3. Classify 10/2. Don't be thrown by the fraction bar. Work out the value first. 10/2 = 5, so its value is 5, which makes it an **integer** (and rational). The spelling looked like a fraction; the value is a whole number.
4. Classify 0.333…. The repeating decimal is exactly 1/3, a ratio of integers, so it's **rational, non-integer**. Again, the value decides, not the look.
5. Show three spellings of one number: 5 = 5/1 = 5.0. All three name the *same point* on the line, a useful reminder that how you write it doesn't change what it is.
6. Classify √2 and π. Neither can be written as a ratio of integers, because their decimals never end and never repeat, so both are **irrational** (and therefore **real**, like every point on the line). Compare √9: it *looks* like a root, but √9 = 3, an **integer**. Value, not spelling.
...
2. 4 + (−9) = −5. Adding a negative steps left: start at 4, walk 9 left.
3. 5 − 8 = 5 + (−8) = −3. Subtracting is adding the opposite, so this is a step 8 to the left of 5.
4. 3 − (−4) = 3 + 4 = 7. Subtracting a negative adds: wiping out a $4 debt leaves you $4 richer. This is the one to slow down on.
5. (−6)(3) = −18. Different signs, so the product is negative.
6. (−6)(−3) = 18. Same signs, so the product is positive.
7. −20 ÷ 4 = −5. Different signs, so the quotient is negative.
8. −20 ÷ (−5) = 4. Same signs, so the quotient is positive.
9. |5| = 5, |−5| = 5, |0| = 0. Each is a distance from 0, so none can be negative.
10. Which is farther from 0, −8 or 3? Compare distances: |−8| = 8 and |3| = 3, so −8 is farther, even though −8 is the smaller *value*. Distance and value are two different questions.
...
*(In 6 and 7 you're subtracting a negative; 15 and 16 are chains of negatives, so track each sign; for 17 to 20, read absolute value as distance from 0, which is always ≥ 0.)*
...
1. Expression or equation? 3x + 2. No equals sign, so it's an **expression**, a phrase that names a value once you know x.
2. Expression or equation? 3x + 2 = 14. It has an =, so it's an **equation**, a sentence claiming the two sides match.
3. Evaluate 3x + 2 at x = 4. The seat holds 4, so 3(4) + 2 = 12 + 2 = 14. Each step written out, no jumping.
4. Evaluate 5 − 2x at x = −1. Keep the −1 in parentheses: 5 − 2(−1) = 5 + 2 = 7. Subtracting a negative adds, so watch that sign.
5. Structure check: evaluate 2(x + 3) and 2x + 3 at x = 3. The first is 2(3 + 3) = 2(6) = 12; the second is 2(3) + 3 = 9. Different values, so they're different expressions; the parentheses change the meaning.
