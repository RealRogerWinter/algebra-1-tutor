...
Before this unit, an equation like y = 2x + 1 had a whole line of answers — every point on that line fits it. This unit adds a second condition that has to hold at the same time, and that one extra demand usually pins the answer down to a single point.

Keep one picture in mind the whole way through. Each equation draws a line. A pair of equations is asking: is there one point that sits on both lines at once? When there is, it's the place the two lines cross. Everything in this unit, whether graphing or substitution or elimination, is just a different way to find that crossing point. And one habit never changes: once you have an answer, put it back into both equations and make sure both come out true.

Before each new lesson here, it helps to redo two or three problems from the lesson before from memory, then check them. Mixed, spaced practice like that feels harder in the moment, and that's exactly what makes a skill last to next week. Every set in this unit has its answers at the end of the lesson, so the help is always right there.
...
That question is what a **system** asks. Two relationships, both true at once. And the answer, when there is one, is the point where the two lines cross — the one (x, y) that satisfies both equations together. From Unit 5 you know a line is really a function. So a system is asking: for what input do these two functions hand back the same output?
...
Look at the row x = 2: both equations give y = 3. So both lines pass through (2, 3), and that shared point is the solution. The graph would show the two lines crossing right there — but you didn't need to eyeball it, because the table told you exactly.
...
1. {#7.2.c1} "In y = 2x and x + y = 9, why is it safe to write x + 2x = 9? What justifies swapping y for 2x?" (Because the first equation says y *equals* 2x — they're the same number — and y is the same number in both equations. Swapping equal things for equal things keeps the second equation true.)
...
3. {#7.2.c3} "Make up a system where substitution is clearly the easier method than elimination, and say why." (Any system with a variable already alone, like y = 3x − 2 and x + y = 10 — you can pour 3x − 2 straight in with no rearranging. When a variable is sitting by itself, substitution is the short path.)
...
The idea starts from the balance scale. Each equation is a level scale: the left side weighs exactly what the right side weighs. So if you have two true equations and you add their left sides together and their right sides together, the result is still balanced — still true. That's the move. You stack two equations and combine them, and if you've lined things up right, one variable disappears in the adding.
...
One thing about these cases surprises people the first time. When you solve a normal system, the variables stay around long enough to give you a value. But in these two special cases, the variables all vanish partway through, and what's left over is the signal that tells you which case you're in. If the leftover statement is **false**, like 0 = 5, no numbers can ever make it true, so there's **no solution**: that's the parallel case. If the leftover is always **true**, like 0 = 0, then every point already works, so there are **infinitely many**: that's the same-line case. So when the x's and y's disappear, don't panic; read the statement they leave behind.
...
Scale the first equation by 2 to match the second's x-term: 2(2x − y) = 2(3) gives 4x − 2y = 6. Now subtract the second equation from it: (4x − 2y) − (4x − 2y) = 6 − 1. The whole left side goes to zero, leaving 0 = 5 — **false**. So **no solution.** (The two lines are parallel; the scaling just made it visible.)
...
*Example 6 — application (mixture).* How many liters of a **20%** salt solution and a **50%** salt solution should you mix to make **30 liters** of a **30%** solution?
...
Let l = length and w = width. The two facts become two equations — and recall perimeter is twice the length plus twice the width:
...
2. {#7.4.c2} "Two lines have the same slope. What two situations are still possible, and how does the algebra tell them apart?" (Either they're parallel with different intercepts — no solution, and the algebra leaves a false statement like 0 = 5 — or they're the same line — infinitely many, and the algebra leaves a true statement like 0 = 0. The leftover statement tells you which.)
3. {#7.4.c3} "A problem says '30 nickels and dimes worth $2.40.' Name your two variables and write the two equations — don't solve yet." (Let n = nickels and d = dimes. One equation counts coins, one counts cents: n + d = 30 and 5n + 10d = 240.)
